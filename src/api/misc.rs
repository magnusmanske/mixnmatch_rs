...
pub async fn query_get_user_info(app: &AppState, params: &Params) -> Result<Response, ApiError> {
    ...
    match app.storage().get_user_by_name(&name).await? {
        ...
        None => Err(ApiError::NotFound(format!("No user '{name}' found"))),
    }
}
...
pub async fn query_set_statement_text_q(
    ...
    if text.is_empty() {
        return Err(ApiError::BadRequest("Missing text parameter".into()));
    }
    if property <= 0 {
        return Err(ApiError::BadRequest("Missing or invalid property parameter".into()));
    }
    if q <= 0 {
        return Err(ApiError::BadRequest("Missing or invalid q parameter".into()));
    }
...
pub async fn query_missingpages(app: &AppState, params: &Params) -> Result<Response, ApiError> {
    ...
    if site.is_empty() {
        return Err(ApiError::BadRequest("site parameter required".into()));
    }
