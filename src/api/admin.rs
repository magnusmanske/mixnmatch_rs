...
pub async fn query_update_ext_urls(
    ...
    if parts.len() != 2 {
        return Err(ApiError::BadRequest(format!("Bad $1 replacement for '{url}'")));
    }
    app.storage()
...
pub async fn query_add_aliases(
    ...
    if cid == 0 || text.is_empty() {
        return Err(ApiError::BadRequest("Catalog ID or text missing".into()));
    }
...
pub async fn query_set_missing_properties_status(
    ...
    if row_id == 0 {
        return Err(ApiError::BadRequest("Bad/missing row ID".into()));
    }
