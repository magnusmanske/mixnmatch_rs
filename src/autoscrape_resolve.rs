...
mod tests {
    ...
    #[test]
    fn test_replace_vars_substitutes_dollar_rid_from_map() {
        // $RID is populated upstream (in process_html_block_generate_entry_ex)
        // from the fully-resolved id field. From replace_vars's point of
        // view it's just another map key, but pin the substitution down
        // here so a map ordering or stripping change can't break it.
        let json = json!({"test":{
            "use": "https://example.com/$RID"
        }});
        let resolve = AutoscrapeResolve::from_json(&json, "test").unwrap();
        let mut map = HashMap::new();
        map.insert("$RID".to_string(), "abc-123".to_string());
        assert_eq!(resolve.replace_vars(&map), "https://example.com/abc-123");
    }
