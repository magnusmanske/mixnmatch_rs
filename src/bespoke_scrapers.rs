use crate::{app_state::AppContext, catalog::Catalog, entry::{Entry, EntryWriter}, meta_entry::MetaEntry};
...
pub mod scraper_4098;
pub mod scraper_4361;
pub mod scraper_4589;
pub mod scraper_4837;
pub mod scraper_4600;
...
pub mod scraper_5335;
pub mod scraper_5347;
pub mod scraper_5522;
pub mod scraper_5959;
...
pub use scraper_4098::BespokeScraper4098;
pub use scraper_4361::BespokeScraper4361;
pub use scraper_4589::BespokeScraper4589;
pub use scraper_4600::BespokeScraper4600;
pub use scraper_4837::BespokeScraper4837;
...
pub use scraper_5335::BespokeScraper5335;
pub use scraper_5347::BespokeScraper5347;
pub use scraper_5522::BespokeScraper5522;
pub use scraper_5959::BespokeScraper5959;
...
pub use scraper_7895::BespokeScraper7895;
...
const SCRAPER_REGISTRY: &[(usize, ScraperRunFn)] = &[
    ...
    scraper_entry!(4098, BespokeScraper4098),
    scraper_entry!(4361, BespokeScraper4361),
    scraper_entry!(4589, BespokeScraper4589),
    scraper_entry!(4600, BespokeScraper4600),
    scraper_entry!(4679, BespokeScraper4679),
    scraper_entry!(4681, BespokeScraper4681),
    scraper_entry!(4825, BespokeScraper4825),
    scraper_entry!(4837, BespokeScraper4837),
    ...
    scraper_entry!(5335, BespokeScraper5335),
    scraper_entry!(5347, BespokeScraper5347),
    scraper_entry!(5522, BespokeScraper5522),
    scraper_entry!(5959, BespokeScraper5959),
    ...
    scraper_entry!(7895, BespokeScraper7895),
];
...
pub trait BespokeScraper {
    ...
    async fn maybe_flush_cache(&self, cache: &mut Vec<MetaEntry>) -> Result<()> {
    ...
    async fn process_cache(&self, entry_cache: &mut Vec<MetaEntry>) -> Result<()> {
            ...
            match existing_entry {
                Some(mut entry) => {
                    if self.keep_existing_names() {
                        meta.entry.ext_name = entry.ext_name.to_string();
                    }
                    if self.testing() {
                        info!("EXISTS: {meta:?}");
                    } else {
                        let entry_id = entry.id;
                        meta.update_merge_in_storage(&mut entry, self.app())
                            .await
                            .with_context(|| format!(
                                "process_cache: update_merge_in_storage failed (catalog={catalog_id}, ext_id={ext_id:?}, entry_id={entry_id:?})"
                            ))?;
                    }
                }
                None => {
                    if self.testing() {
                        ...
                        meta.create_in_storage(self.app())
                            .await
                            .with_context(|| format!(
                                "process_cache: create_in_storage failed (catalog={catalog_id}, ext_id={ext_id:?})"
                            ))?;
                    }
                }
            };
