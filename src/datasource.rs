...
impl DataSource {
    ...
    async fn fetch_url(&self, url: &String, file_name: &Path) -> Result<()> {
        ...
        std::fs::write(file_name, Self::strip_utf8_bom(&bytes))?;
        Ok(())
    }

    /// Returns `bytes` with the UTF-8 BOM (`0xEF 0xBB 0xBF`) stripped from the
    /// front, if present. Import files generated by Windows tools commonly carry
    /// a BOM, which would otherwise become part of the first field value.
...
mod tests {
    ...
    #[test]
    fn test_strip_utf8_bom_removes_bom() {
        let with_bom = b"\xEF\xBB\xBFhello,world";
        assert_eq!(DataSource::strip_utf8_bom(with_bom), b"hello,world");
    }

    #[test]
    fn test_strip_utf8_bom_no_bom() {
        let no_bom = b"hello,world";
        assert_eq!(DataSource::strip_utf8_bom(no_bom), b"hello,world");
    }

    #[test]
    fn test_strip_utf8_bom_only_bom() {
        let only_bom = b"\xEF\xBB\xBF";
        assert_eq!(DataSource::strip_utf8_bom(only_bom), b"");
    }

    #[test]
    fn test_strip_utf8_bom_partial_bom() {
        let partial = b"\xEF\xBBhello";
        assert_eq!(DataSource::strip_utf8_bom(partial), b"\xEF\xBBhello");
    }
}
